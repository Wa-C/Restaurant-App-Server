...
*********************************/
app.get('/' , (req, res) => {
    res.send("Welcome to Restaurent-App Backend Server");
})
...
app.get('/foods' , (req, res) => {
    ...
    client.connect(err => {
        if(err){
            ...
            const collection = client.db('RestaurentApp').collection('foods');
...
app.get('/features' , (req,res) => {
    client = new MongoClient(uri , {useNewUrlParser:true , useUnifiedTopology: true});
    client.connect(err => {
        const collection = client.db('RestaurentApp').collection('features');
        collection.find().toArray((rej,documents) => {
            if(rej){
...
app.post('/submitorder' , (req,res) => {
    ...
    client = new MongoClient(uri , {useNewUrlParser:true , useUnifiedTopology: true});
    client.connect(err => {
        const collection = client.db('RestaurentApp').collection('orders');
